...
answer = reverse $ sumOfLargeIntegers largeIntegersdD

sumOfLargeIntegers :: [[Int]] -> [Int]
sumOfLargeIntegers a = foldr (\x acc -> fullAdder x acc 0) [] a

fullAdder :: [Int] -> [Int] -> Int -> [Int]
fullAdder [] [] carry = if carry == 0 then [] else [carry]
fullAdder [] (b:bs) carry = y:fullAdder [] bs x
    where (x,y) = simpleAdderWithCarry 0 b carry
fullAdder (a:as) [] carry = y:fullAdder as [] x
    where (x,y) = simpleAdderWithCarry 0 a carry
fullAdder (a:as) (b:bs) carry = y:fullAdder as bs x
    where (x,y) = simpleAdderWithCarry a b carry

simpleAdderWithCarry :: Int -> Int -> Int -> (Int, Int)
simpleAdderWithCarry a b c = (carry, add)
    where add   = (a+b+c) `mod` 10
          carry = (a+b+c) `div` 10

largeIntegersdD :: [[Int]]
largeIntegersdD = map reverse largeIntegersDd
largeIntegersDd = [
